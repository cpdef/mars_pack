...
local function furnace_node_timer(pos, elapsed)
	...
	local oxy_pos = minetest.find_node_near(pos, 1, "gas:oxygen")
	local have_oxygen = false
	if oxy_pos ~= nil and gas.get_concentration(oxy_pos) > 0 then
		have_oxygen = true
	end
	...
	while elapsed > 0 and update do
		...
		if fuel_time < fuel_totaltime and have_oxygen then
			-- The furnace is currently active and has enough fuel
			gas.add_concentration(oxy_pos, "oxygen", -1)
			...
			if cookable and have_oxygen then
	...
	if fuel_totaltime ~= 0 then
		...
		swap_node(pos, "marssurvive:furnace_active")
		...
		swap_node(pos, "marssurvive:furnace")
...
minetest.register_node("marssurvive:furnace", {
	description = S("Mars Furnace"),
	tiles = {
	...
	on_blast = function(pos)
		...
		drops[#drops+1] = "marssurvive:furnace"
...
minetest.register_node("marssurvive:furnace_active", {
...
minetest.register_craftitem("marssurvive:coal_piece", {
	description = S("Coal Lump"),
	inventory_image = "coal_piece.png",
	groups = {coal = 1, flammable = 1}
})

minetest.register_craft({
	output = "marssurvive:coal_piece 40",
	recipe = {
		{"default:coal_lump"},
	}
})
